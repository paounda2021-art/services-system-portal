# PowerShell Static Web Server for the FMO Systems Portal (Root)
# Run this script to serve the entire portal locally (including index.html, KPI, and Car Booking).
...
Write-Host "=========================================================="
Write-Host "  FMO Systems Portal Local Web Server"
Write-Host "  Listening on: http://localhost:$port/"
Write-Host "  Root Directory: $rootDir"
Write-Host "  To stop the server, press Ctrl+C in this console."
Write-Host "=========================================================="

# Launch the default browser to open the portal
    ...
    while ($listener.IsListening) {
        ...
        if ($urlPath -eq "/api/send-email" -and $request.HttpMethod -eq "POST") {
            try {
                ...
                # Find smtp_config.json in root or in deploy_latest
                $configPath = Join-Path $rootDir "smtp_config.json"
                if (-not (Test-Path $configPath)) {
                    $configPath = Join-Path $rootDir "deploy_latest\smtp_config.json"
                }
                
                if (-not (Test-Path $configPath)) {
                    throw "SMTP configuration file (smtp_config.json) not found."
                }
